...
if [ -e /var/lib/rancher/k3s/server/db/etcd ]; then
  ...
  # FIX: Remove reset-flag and etcd directories after successful cluster-reset
  echo "INFO: Cluster reset successful, cleaning up reset artifacts"
  rm -f /var/lib/rancher/k3s/server/db/reset-flag
  rm -rf /var/lib/rancher/k3s/server/db/etcd
  rm -rf /var/lib/rancher/management-state/etcd
  set -e
fi
